export const SQL_GET_BIBLE_CHAPTERS_PAGED = `
-- ===========================
-- Paginated Bible Chapters Function
-- ===========================

CREATE OR REPLACE FUNCTION get_bible_chapters_paged(
    book_id UUID,
    page_number INTEGER DEFAULT 1,
    page_size INTEGER DEFAULT 20
)
RETURNS JSON
LANGUAGE plpgsql
AS $$
DECLARE
    total_items INTEGER;
    total_pages INTEGER;
    offset_value INTEGER;
    current_page INTEGER;
    items_on_current_page INTEGER;
    chapters_data JSON;
    book_data JSON;
    translation_data JSON;
    pagination_info JSON;
    result JSON;
BEGIN
     -- Validate inputs
    IF page_number < 1 THEN
        page_number := 1;
    END IF;
    
    IF page_size < 1 THEN
        page_size := 20;
    END IF;
    
    current_page := page_number;

    -- Get total count of books for this bible
    SELECT COUNT(*)
    INTO total_items
    FROM bible_chapter bc
    WHERE bc.bible_book_id = get_bible_chapters_paged.book_id;

    -- Calculate pagination values
    total_pages := CASE 
        WHEN total_items = 0 THEN 1 
        ELSE CEIL(total_items::DECIMAL / page_size) 
    END;
    
    offset_value := (current_page - 1) * page_size;
    
    -- Ensure current_page doesn't exceed total_pages
    IF current_page > total_pages THEN
        current_page := total_pages;
        offset_value := (current_page - 1) * page_size;
    END IF;

    -- Get paginated chapters
    -- Get paginated chapters with proper ordering
    SELECT json_agg(row_to_json(bc_ordered.*))
    INTO chapters_data
    FROM (
        SELECT bc.*
        FROM bible_chapter bc
        WHERE bc.bible_book_id = get_bible_chapters_paged.book_id
        ORDER BY bc.number  -- Add ordering for consistent results
        LIMIT page_size
        OFFSET offset_value
    ) bc_ordered;

    -- Getting translation details
    SELECT row_to_json(bt.*)
    INTO translation_data
    FROM bible_translation bt 
    WHERE bt.id = (SELECT translation_id 
                FROM bible 
                WHERE id = (SELECT bible_id
                            FROM bible_book
                            WHERE id = get_bible_chapters_paged.book_id
                            )
                );


    -- Embed bible data into result 
    SELECT row_to_json(bb.*)
    INTO book_data
    FROM bible_book bb
    WHERE bb.id = get_bible_chapters_paged.book_id;


    -- Handle case where no books found
    IF chapters_data IS NULL THEN
        chapters_data := '[]'::json;
    END IF;
    
    -- Get actual count of items returned
    SELECT json_array_length(chapters_data) INTO items_on_current_page;
    
    -- Build pagination info
    pagination_info := json_build_object(
        'current_page', current_page,
        'page_size', page_size,
        'total_items', total_items,
        'total_pages', total_pages,
        'min_page', 1,
        'max_page', total_pages,
        'has_next', current_page < total_pages,
        'has_previous', current_page > 1,
        'next_page', CASE 
            WHEN current_page < total_pages THEN current_page + 1 
            ELSE NULL 
        END,
        'previous_page', CASE 
            WHEN current_page > 1 THEN current_page - 1 
            ELSE NULL 
        END,
        'first_page', 1,
        'last_page', total_pages,
        'items_on_current_page', items_on_current_page,
        'offset', offset_value,
        'is_first_page', current_page = 1,
        'is_last_page', current_page = total_pages
    );
    
    -- Build final result
    result := json_build_object(
        'items', chapters_data,
        'book_data', book_data,
        'translation_data', translation_data,
        'pagination', pagination_info
    );

    RETURN result;
END;
$$;
`;
